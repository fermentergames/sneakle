...
if global.show_any_modal_fd < 0.5 {
	...
	if global.game_phase = 2 || global.game_phase = 3 {
		...
		if obj_ctrl.selecting >= 1 {
			if am_selected = 0 {
				if point_distance(x,y,mouse_x,mouse_y) < swipe_radius {
				
					var _close_enough_to_neighbor = 0 
				
					if instance_exists(obj_ctrl.selected_word_latest_tile_id) {
						if point_distance(x,y,obj_ctrl.selected_word_latest_tile_id.x,obj_ctrl.selected_word_latest_tile_id.y) <= 100 {
							_close_enough_to_neighbor = 1
						}
					}
				
				
					if _close_enough_to_neighbor = 1 {
						am_selected = 1
						am_selected_flash = 1
						...
						audio_play_sound(snd_mm_click_003,0,0,0.11,0,0.2+(0.04*obj_ctrl.selected_word_length))
				...
				if point_distance(x,y,mouse_x,mouse_y) < swipe_radius {
					...
					audio_play_sound(snd_mm_click_003,0,0,0.08,0,0.2+(0.04*obj_ctrl.selected_word_length))
